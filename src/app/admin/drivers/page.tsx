        ...
        <nav className="flex-1 p-4 space-y-2">
            {menuItems.map(item => (
                <Link key={item.href} href={item.href}>
                    <span className={`flex items-center gap-3 rounded-lg px-3 py-2 text-muted-foreground transition-all hover:bg-accent/20 hover:text-primary ${item.href.includes('drivers') ? 'bg-accent/20 text-primary' : ''}`}>
                  ...
                  {verifiedDrivers.map(driver => (
                    <TableRow key={driver.id}>
                      ...
                      <TableCell>
                          ...
                          <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive/80">Suspend</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
